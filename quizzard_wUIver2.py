...
- "answer": the correct answer ("A"/"B"/"C"/"D" for mcq, exact text for enum)
- "explanation": A helpful explanation of why the correct answer is right and, if applicable, why common misconceptions/wrong options are incorrect.
"""
...
class QuizzardApp:
    def __init__(self, root):
        ...
        self.root.geometry("900x700") # Slightly taller for the results
        self.root.config(bg="#F7F8FF")

        self.gemini = GeminiClient(model="models/gemini-1.5-flash")
        ...
        self.user_answers: List[Dict[str, Any]] = [] # Store user responses
    ...
    def build_subject_screen(self):
        ...
        def on_subject_change(*args):
            self.apply_subject_theme()

        self.subject.trace_add("write", on_subject_change)
        self.apply_subject_theme()
    ...
    def generate_quiz(self):
        ...
        def worker():
            ...
            try:
                ...
                if not questions:
                    questions = [{"type":"enum","text":"Error: Could not parse questions from Gemini API.","answer":"","explanation":""}]
            except Exception as e:
                questions = [{"type":"enum","text":f"Error calling Gemini API: {e}","answer":"","explanation":""}]
    ...
    def show_score(self):
        ...
        # Header
        header_frame = tk.Frame(self.root, bg="#F7F8FF")
        header_frame.pack(pady=10)
        tk.Label(header_frame, text="Quiz Completed!", font=("Helvetica",24,"bold"), bg="#F7F8FF").pack()
        tk.Label(header_frame, text=f"Final Score: {self.score} / {len(self.questions)}", font=("Arial",18), bg="#F7F8FF", fg="#323279").pack(pady=5)

        # Scrollable Report Area
        report_frame = tk.Frame(self.root)
        report_frame.pack(fill="both", expand=True, padx=20, pady=10)

        txt = scrolledtext.ScrolledText(report_frame, wrap="word", font=("Arial", 11))
        txt.pack(fill="both", expand=True)
        
        # Configure tags for coloring text
        txt.tag_config("correct", foreground="green", font=("Arial", 11, "bold"))
        txt.tag_config("wrong", foreground="red", font=("Arial", 11, "bold"))
        txt.tag_config("header", font=("Arial", 12, "bold"))
        txt.tag_config("explanation", foreground="#444444", font=("Arial", 11, "italic"))

        # Populate Report
        for i, res in enumerate(self.user_answers):
            q_num = i + 1
            txt.insert("end", f"Question {q_num}: ", "header")
            txt.insert("end", f"{res['question']}\n")
            
            if res['is_correct']:
                txt.insert("end", "Result: Correct ✅\n", "correct")
                txt.insert("end", f"Your Answer: {res['user_answer']}\n")
            else:
                txt.insert("end", "Result: Incorrect ❌\n", "wrong")
                txt.insert("end", f"Your Answer: {res['user_answer']}  |  Correct Answer: {res['correct_answer']}\n", "wrong")

            # Explanation section
            txt.insert("end", "Explanation: ", "header")
            txt.insert("end", f"{res['explanation']}\n", "explanation")
            txt.insert("end", "-"*60 + "\n\n")

        txt.configure(state="disabled") # Make read-only

        ttk.Button(self.root, text="Back to Landing Page", command=self.build_first_page).pack(pady=10)

    # ---------- Subject Themes ----------
    SUBJECT_THEMES = {
        "Mathematics": {"bg": "#E3F2FD", "accent": "#1565C0"},
        "Science": {"bg": "#E8F5E9", "accent": "#2E7D32"},
        "English": {"bg": "#FFF3E0", "accent": "#EF6C00"},
        "Filipino": {"bg": "#FCE4EC", "accent": "#C2185B"},
        "Arts": {"bg": "#F3E5F5", "accent": "#6A1B9A"},
        "PE": {"bg": "#E0F7FA", "accent": "#00838F"},
        "Social Studies": {"bg": "#FBE9E7", "accent": "#D84315"},
        "ICT": {"bg": "#ECEFF1", "accent": "#37474F"},

        "STEM": {"bg": "#E1F5FE", "accent": "#0277BD"},
        "ABM": {"bg": "#F1F8E9", "accent": "#558B2F"},
        "HUMSS": {"bg": "#FFFDE7", "accent": "#F9A825"},
        "GAS": {"bg": "#EDE7F6", "accent": "#512DA8"},
        "Programming": {"bg": "#0F172A", "accent": "#38BDF8"},
        "Data Structures": {"bg": "#1E293B", "accent": "#22C55E"},
        "Algorithms": {"bg": "#111827", "accent": "#F59E0B"},
        "Computer Networks": {"bg": "#020617", "accent": "#A855F7"},
    }

    def apply_subject_theme(self):
        theme = self.SUBJECT_THEMES.get(
            self.subject.get(),
            {"bg": "#F7F8FF", "accent": "#323279"}
        )
        bg = theme["bg"]
        accent = theme["accent"]
        ...
        style.configure("TRadiobutton", background=bg, font=("Arial", 12))
